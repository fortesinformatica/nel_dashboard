...
SCHEDULER.every '60s' do
  ...
  send_event('atlas_rollbar_errors', { items: atlas } )
  send_event('leitor10_rollbar_errors', { items: leitor10 } )
end
...
def rollbar_errors access_token
  ...
  [
    get_count_by(items, 'Errors'),
    get_count_by(items, 'Warnings'),
    get_count_by(items, 'Infos')
  ]
end
